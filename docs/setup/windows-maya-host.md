...
session identity, proves Maya tool readiness with a read-only `scene.info` call
and a small `viewport.capture`, and refuses to reuse the previous identity. A
running daemon and ready call server alone do not prove that Maya's commandPort
and viewport have finished initializing. The same task
